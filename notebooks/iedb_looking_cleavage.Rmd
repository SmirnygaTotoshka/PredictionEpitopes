...
output: 
  html_document:
    ...
    toc_float:
      collapsed: false
      smooth_scroll: false
bibliography: bib/references_look_iedb_cleavage.bib
---

# Введение

The Immune Epitope Database (IEDB)@vita2019 - крупнейшая база данных по Т и В-клеточным эпитопам. Необходимо оценить частоту встречаемости аминокислот на С-конце эпитопа, так как с него как правило не идет процессинг антигена для МНС 1 класса и его можно считать сайтом разрезания протеасомы.
...
#Определение необходимых источников данных и условий фильтрации
...
Для каждой записи есть ссылка на источник.
...
```{r}
table(all.assays$as_char_value)
```

Берем С-концевые аминокислоты эпитопов и +1 после неё, разделив на положительные и отрицательные случаи

```{r}
neg.assays = subset.data.frame(all.assays, as_char_value == "Negative")[,c("linear_peptide_seq","sequence")]
pos.assays = subset.data.frame(all.assays, as_char_value != "Negative")[,c("linear_peptide_seq","sequence")]
```

## Положительные случаи

```{r}
locations = stringr::str_locate_all(pos.assays$sequence, pos.assays$linear_peptide_seq)
```

```{r}
C.terms = lapply(seq_along(locations),function(i, seq){
    str_sub(pos.assays$sequence[i],locations[[i]][,"end"],locations[[i]][,"end"]+1)
})
```
...
epitopes = unique(pos.assays$linear_peptide_seq)
print(length(epitopes))
...
ax.set_title("IEDB positive epitopes")
logo = logomaker.Logo(probs.iloc[0:15,:], color_scheme = "chemistry", ax = ax)
fig.tight_layout()
plt.show()
```

Можно заметить, что на N-конце распределение остатков более менее равномерное, когда на C-конце преобладают гидрофобные и основные остатки.

```{r}
plot(round(table(pairs.df$V1) / nrow(pairs.df), 5),ylab = "Частота", xlab = "Аминокислота")
```

```{r}
plot(round(table(pairs.df$V2) / nrow(pairs.df), 5),ylab = "Частота", xlab = "Аминокислота")
```

Из этих двух графиков видно, что не так важна какая аминокислота идет после сайта разрезания, важно что разрезание идет по гидрофобным и положительно заряженным аминокислотам.

## Отрицательные случаи

```{r}
locations = stringr::str_locate_all(neg.assays$sequence, neg.assays$linear_peptide_seq)
```

```{r}
C.terms = lapply(seq_along(locations),function(i, seq){
    str_sub(neg.assays$sequence[i],locations[[i]][,"end"],locations[[i]][,"end"]+1)
})
```
...
probs = data.frame(matrix(0,nrow = 20,ncol = 20))
colnames(probs) = unlist(str_split("ACDEFGHIKLMNPQRSTVWY",pattern = ""))
epitopes = unique(neg.assays$linear_peptide_seq)
print(length(epitopes))
for (i in seq_len(nrow(probs))) {
  p = table(sapply(epitopes,str_sub,i,i))
  for (j in colnames(probs)) {
...
probs = r.probs
fig,ax = plt.subplots(figsize = (10,6))
ax.set_title("IEDB negative epitopes")
logo = logomaker.Logo(probs.iloc[0:10,:], color_scheme = "chemistry", ax = ax)
fig.tight_layout()
plt.show()
```
...
Для отрицательных случаев получились похожие результаты.

Удалить созданные промежуточные таблицы

```{sql connection=con}
DROP TABLE IF EXISTS filtered_bind, filtered_elution, filtered_epitope_cur,filtered_mhc, epi_mhc_bind,epi_mhc_elution;
```
